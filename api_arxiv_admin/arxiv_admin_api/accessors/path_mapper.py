...
def local_path_to_blob_key(local_path: str) -> str:
    """Convert a local path to a GCP bucket key.
    ...
    if str(local_path).startswith(CONSTANTS.CACHE_PREFIX + "/"):
        ...
        logger.error(
            "path_to_bucket_key: %s does not start with %s or %s",
            local_path,
...
def arxiv_id_to_local_pdf_path(arxiv_id: arXivID, extent: str = ".pdf", prefix: str = "") -> str:
    """Convert ID to local path for PDFs."""
    archive = "arxiv" if not arxiv_id.is_old_id else arxiv_id.archive
    # noinspection LongLine
    if arxiv_id.has_version:
        return f"{CONSTANTS.PS_CACHE_PREFIX}/{archive}/pdf/{arxiv_id.yymm}/{prefix}{arxiv_id.filename}v{arxiv_id.version}{extent}"  # noqa: E501
...
def arxiv_id_to_local_orig(arxiv_id: arXivID, extent: str = ".abs", prefix: str = "") -> str:
    ...
    # noinspection LongLine
    if arxiv_id.has_version:
        return f"{CONSTANTS.ORIG_PREFIX}/{archive}/papers/{arxiv_id.yymm}/{prefix}{arxiv_id.filename}v{arxiv_id.version}{extent}"  # noqa: E501
...
def arxiv_id_to_pdf_url(host: str, arxiv_id: arXivID) -> str:
    """Map arXiv IDs to webX url.

    Since this has nothing to do with GCP, it is kind of an alien here.
    """
    if arxiv_id.has_version:
        return f"https://{host}/pdf/{arxiv_id.idv}"
    return f"https://{host}/pdf/{arxiv_id.id}"
