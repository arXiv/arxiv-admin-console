...
@pytest.mark.usefixtures("reset_test_database", "database_session")
class TestReadOnlys:

    def test_see_papers(self, database_session):
    ...
    def test_academic_email(self, database_session):
        with database_session() as db_session:
            accessor = EndorsementDBAccessor(db_session)
    ...
    def test_is_moderator(self, database_session):
        with database_session() as db_session:
            accessor = EndorsementDBAccessor(db_session)
            user_ids = db_session.query(TapirUser.user_id).all()
            count = 0
    ...
    def test_get_category(self, database_session):
        with database_session() as db_session:
            accessor = EndorsementDBAccessor(db_session)
    ...
    def test_get_domain_info(self, database_session):
        with database_session() as db_session:
            accessor = EndorsementDBAccessor(db_session)
    ...
    def test_get_endorsements(self, database_session):
        with database_session() as db_session:
            accessor = EndorsementDBAccessor(db_session)
            user_ids = db_session.query(TapirUser.user_id).all()
            count = 0
    ...
    def test_get_questionable_categories(self, database_session):
        with database_session() as db_session:
            accessor = EndorsementDBAccessor(db_session)
            count = 0
    ...
    def test_get_papers_by_user(self, database_session):
        with database_session() as db_session:
            accessor = EndorsementDBAccessor(db_session)
            user_ids = db_session.query(TapirUser.user_id).all()
            count = 0
...
def test_create_endorsement(database_session, test_audit_timestamp, test_tapir_session_id):
    ...
    with database_session() as session:
        ...
        er1 = EndorsementRequestModel.base_select(session).filter(
            EndorsementRequest.secret == endorsement_code
        ).one_or_none()
