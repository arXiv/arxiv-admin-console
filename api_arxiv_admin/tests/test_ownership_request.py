...
class TestCreateOwnershipRequest:
    ...
    def test_create_ownership_request(self, arxiv_db, db_session: Session, test_env, api_client, api_headers):
        ...
        row: OwnershipRequest = db_session.query(OwnershipRequest).filter(
            OwnershipRequest.request_id == ownership_request_id).one_or_none()
        assert row is not None

        or_audit: OwnershipRequestsAudit | None = db_session.query(OwnershipRequestsAudit).filter(
        ...
        response11 = api_client.get(f"/v1/paper_owners?user_id=1", headers=api_headers)
        result11 = response11.json()
        assert len(result11) == 0

        # Now update the request jest created
        response2 = api_client.get("/v1/ownership_requests/1", headers=api_headers)
        ...
        response3 = api_client.put("/v1/ownership_requests/1", headers=api_headers,
        ...
        response31 = api_client.get(f"/v1/paper_owners?user_id=1", headers=api_headers)
        ...
        result31_sorted = sorted(result31, key=lambda x: x['document_id'])
        expected_result_sorted = sorted(expected_result, key=lambda x: x['document_id'])
